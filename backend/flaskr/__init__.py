...
def create_app(test_config=None):
    ...
    def after_request(response):
        ...
        response.headers.add(
            "Access-Control-Allow-Methods", "GET,POST,DELETE,OPTION,PUT"
        )
        return response
    ...
    def get_categories():
        ...
        if len(categories) == 0:
            abort(404)
        return jsonify({"success": True, "categories": get_category_dict(categories)})
    ...
    def search_question():
        ...
        if "searchTerm" in body and body["searchTerm"] != "":
            ...
            found_items = Question.query.filter(
    ...
    def play_quizzes():
        try:
            ...
            if total_question_result > 0:
                ...
                question_result = list(map(lambda item: item.format(), questions))[
                    random_number
                ]

            return jsonify({"success": True, "question": question_result})
    ...
    def unable_process(error_code):
        return (
            jsonify({"success": False, "error": 422, "message": "unable process"}),
            422,
        )
